...
CREATE TABLE IF NOT EXISTS hub.event_visitor_tiers (
...
CREATE TABLE IF NOT EXISTS hub.event_invitations (
  id              uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  event_id        uuid NOT NULL REFERENCES hub.events(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS event_invitations_invite_token_idx ON hub.event_invitations(invite_token);
CREATE INDEX IF NOT EXISTS event_invitations_event_id_idx ON hub.event_invitations(event_id);

CREATE TABLE IF NOT EXISTS hub.event_visitors (
  id              uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  event_id        uuid NOT NULL REFERENCES hub.events(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS event_visitors_session_token_idx ON hub.event_visitors(session_token);
CREATE INDEX IF NOT EXISTS event_visitors_event_email_idx ON hub.event_visitors(event_id, email);

CREATE TABLE IF NOT EXISTS hub.event_visitor_favorites (
  id              uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS hub.event_polls (
  id              uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  event_id        uuid NOT NULL REFERENCES hub.events(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS hub.event_poll_answers (
  id              uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS hub.event_visitor_bonus_log (
  id              uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  visitor_id      uuid NOT NULL REFERENCES hub.event_visitors(id) ON DELETE CASCADE,
  amount          int NOT NULL,
...
DROP POLICY IF EXISTS "tiers_organizer" ON hub.event_visitor_tiers;
...
DROP POLICY IF EXISTS "invitations_organizer" ON hub.event_invitations;
...
DROP POLICY IF EXISTS "visitors_organizer" ON hub.event_visitors;
...
DROP POLICY IF EXISTS "visitors_organizer_update" ON hub.event_visitors;
...
DROP POLICY IF EXISTS "visitors_organizer_delete" ON hub.event_visitors;
...
DROP POLICY IF EXISTS "favorites_organizer" ON hub.event_visitor_favorites;
...
DROP POLICY IF EXISTS "polls_read" ON hub.event_polls;
CREATE POLICY "polls_read" ON hub.event_polls
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "polls_organizer" ON hub.event_polls;
...
DROP POLICY IF EXISTS "poll_answers_organizer" ON hub.event_poll_answers;
