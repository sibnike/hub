...
export async function sendInvitation(
  ...
  const adminOrigin =
    process.env.NEXT_PUBLIC_VITRINA_ADMIN?.replace(/\/$/, '') ??
    'https://admin.microp.app'

  await resend.emails.send({
    from: 'Microp <hub@microp.app>',
    to: email,
    ...
    html: `
      ...
      <p>Если у вас ещё нет профиля компании — создайте его на <a href="${adminOrigin}">${adminOrigin}</a> и затем подключитесь к выставке.</p>
    `,
  })
}
