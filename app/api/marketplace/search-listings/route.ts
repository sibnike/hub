...
export async function POST(request: NextRequest) {
  ...
  if (!checkRateLimit(`marketplace-search-listings:${ip}`, 30, 60_000)) {
  ...
  try {
    ...
    const results = await searchListingCache(filter, limit)
    ...
    console.error('[marketplace/search-listings]', message)
